...
public:
		...
		_stateStack(State::Context(_window, _textures, _fonts, _musics, _sounds))
	{
		...
		//_textures.load(Textures::Hexagon, TEXTURES_FOLDER + "Hexagon.png");
		
		auto menuState = std::make_shared<MenuState>(_stateStack, State::Context(_window, _textures, _fonts, _musics, _sounds));
		auto gameState = std::make_shared<GameState>(_stateStack, State::Context(_window, _textures, _fonts, _musics, _sounds));
		_stateStack.pushState(gameState);
	}
